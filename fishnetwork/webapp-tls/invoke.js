...
var orderertlscacert_path = path.resolve(firstnetwork_path, 'crypto-config', 'ordererOrganizations', 'example.com', 'tlsca', 'tlsca.example.com-cert.pem');
...
// create the key value store as defined in the fabric-client/config/default.json 'key-value-store' setting
    Fabric_Client.newDefaultKeyValueStore({
        ...
        return fabric_client.getUserContext('peer1org1', true);
    }).then((user_from_store) => {
        if (user_from_store && user_from_store.isEnrolled()) {
            console.log('Successfully loaded peer1org1 from persistence');
            member_user = user_from_store;
        } else {
            throw new Error('Failed to get peer1org1.... run registerUser.js');
        }
